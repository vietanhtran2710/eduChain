    ...
    Role
{
    ...
    function mintSKILL(address _account, uint256 _amount, bytes memory data) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _mint(_account, SKILL_ID, _amount, data);
    }

    function mintVND(address _account, uint256 _amount, bytes memory data) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _mint(_account, VND_ID, _amount, data);
    }

    function burnVND(address _account, uint256 _amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _burn(_account, VND_ID, _amount);
    }
